...
describe("9 - Crie um botão para remover uma carta do baralho", () => {
  ...
  it("Será validado se ao adicionar uma carta e excluí-la em seguida, a carta não é renderizada", () => {
    ...
    userEvent.type(
      descInput,
      "Bebedouro simpático e com forte pressão na água, ideal para aeroportos"
    );
    userEvent.type(imgInput, "url-to-image");
    userEvent.type(attr1Input, "90");
    userEvent.type(attr2Input, "90");
    ...
    expect(
      screen.getByText("Carta 1 - Bebedouro de Guarulhos")
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Carta 1 - Bebedouro de Guarulhos")
  ...
  it("Será validado se ao adicionar duas cartas e excluir uma em seguida, a carta não é renderizada", () => {
    ...
    userEvent.type(
      descInput,
      "Um pombo por vezes injustiçado mas sempre de bem com a vida"
    );
    userEvent.type(imgInput, "url-to-image");
    userEvent.type(attr1Input, "90");
    userEvent.type(attr2Input, "90");
    ...
    userEvent.type(
      descInput,
      "Em muitas culturas considerada o simbolo de paz, mas nós sabemos sua verdadeira intenção"
    );
    userEvent.type(imgInput, "url-to-image");
    userEvent.type(attr1Input, "90");
    userEvent.type(attr2Input, "90");
    ...
    expect(
      screen.queryByText("Carta 1 - Pombo da Cidade")
    ).not.toBeInTheDocument();
  ...
  it("Será validado se ao excluir uma carta Super Trunfo, o checkbox no formulário é renderizado novamente", () => {
    ...
    userEvent.type(
      descInput,
      'Clássico celular, um dos mais vendidos nos anos 2000 e ainda tem o "Hello Motor"'
    );
    userEvent.type(imgInput, "url-to-image");
    userEvent.type(attr1Input, "90");
    userEvent.type(attr2Input, "90");
