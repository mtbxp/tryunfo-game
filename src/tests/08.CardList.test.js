...
describe("8 - Exiba a lista de cartas que estão salvas no estado", () => {
  it("Será validado se a carta é renderizada na página ao salvá-la", () => {
    ...
    expect(
      screen.getByText(/Um chinelo lendário para pessoas guerreiras/i)
    ).toBeInTheDocument();
  });
  ...
  it("Será validado se a lista é atualizada automaticamente quando uma carta é adicionada", () => {
    ...
    userEvent.type(
      descInput,
      "Um apontador sustentável que recolhe os restos do lápis em sua cesta"
    );
    userEvent.type(attr1Input, "90");
    userEvent.type(attr2Input, "90");
    userEvent.type(attr3Input, "30");
    ...
    expect(
      screen.getByText(/Carta 1 - Apontador com cestinha/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Um apontador sustentável que recolhe os restos do lápis em sua cesta/i
      )
    ).toBeInTheDocument();

    expect(
      screen.getByText(/Carta 2 - Apontador Elétrico/i)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Apontador ideal para membros da casa Sonserina/i)
    ).toBeInTheDocument();
  });
});
